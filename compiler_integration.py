...
def compile(file_path):
    try:
        ...
        return (f"Ran into an error during compilation \n {result.stderr} ")
...
def check_and_generate_IR(model, file_path, FEATURE_LIST = FEATURE_LIST):
    ...
    if(pred==0):
        if(valid_c_file(file_path)):
            ...
            print("OUTPUT: ")
            output= compile(file_path)
            print (output)
            return output
        else:
            return "The file could contain harmful libraries hence not proceeding with compilation"
    else:
        return f"{file_path} is a malicious file thus not proceeding with compilation"
...
if __name__ == "__main__":
    ...
    check_and_generate_IR(model,file_path=file_path)
    # node_kinds = parse_c_file(file_path)
    # xvec = parser_to_vector(node_kinds, FEATURE_LIST).reshape(1, -1)
    # pred = model.predict(xvec)[0]
    # if pred == 0 and valid_c_file(file_path):
    #     print(f"Selected benign file: {file_path}",flush = True)
